...
Trains the full DualBranchModel under three initialisation conditions:
  ...
  transfer  – backbone pretrained on PhysioNet EEG (Stage 7 checkpoint, frozen backbone)

Requires the Stage 7 checkpoint for the 'transfer' condition.

Output:
  <run-dir>/results/real_finetune_scratch_vit.json
  <run-dir>/results/real_finetune_imagenet_vit.json
  <run-dir>/results/real_finetune_transfer_vit.json
  <run-dir>/plots/stage_08_vit/  (per-fold training curves + confusion matrices)

Usage::

    uv run python scripts/pipeline/stage_08_finetune.py --run-dir runs/my_run
    uv run python scripts/pipeline/stage_08_finetune.py --run-dir runs/my_run \\
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Stage 8: Finetune comparison (scratch / imagenet / transfer).",
...
def main() -> None:
    ...
    log = setup_logging(run_dir, f"stage_08_finetune_{bshort}.log")
    log.info("Backbone: %s  (short: %s)", backbone, bshort)

    # Default checkpoint path uses backbone-specific name from Stage 7
    checkpoint_path = (
    ...
    for condition in args.conditions:
        ...
        plots_dir = run_dir / "plots" / f"stage_08_{bshort}_{condition}"
        ...
        result = CVResult(
            strategy="within_subject", model_name=f"{MODEL_NAME}-{condition}", folds=all_folds
        )
        ...
        try:
            ...
            ax.set_title(f"Stage 8 [{condition.upper()}] – {MODEL_NAME} per-subject accuracy")
            ax.set_ylim(0, 105)
            ax.legend()
