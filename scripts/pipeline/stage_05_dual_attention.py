...
Trains the full DualBranchModel (ViT-Tiny + CSP + Riemannian) with
attention fusion. Runs 5-fold within-subject CV and LOSO CV.

Output:
  <run-dir>/results/real_dual_branch_attention_vit.json
  <run-dir>/results/real_dual_branch_attention_loso_vit.json
  <run-dir>/plots/stage_05_vit/  (training curves + confusion matrices)
...
def run_dual_branch(
    ...
    def train_eval(fold_idx, subject_id, X_train, y_train, X_test, y_test):
        ...
        log.info(
            "  Fold %d [S%s]: acc=%.2f%%  kappa=%.3f", fold_idx, sid_str, fr.accuracy, fr.kappa
        )
    ...
    if strategy == "within_subject":
        ...
        for sid, (X, y) in sorted(subject_data.items()):
            ...
            for train_idx, test_idx in skf.split(X, y):
                fr = train_eval(
                    fold_counter, sid, X[train_idx], y[train_idx], X[test_idx], y[test_idx]
                )
                all_folds.append(fr)
                fold_counter += 1
    else:
