...
present) and prints a summary of available subjects.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--run-dir",  default=None,
                   help="Run directory for logs (optional; created if absent)")
    return p.parse_args()
...
def main() -> None:
    ...
    # ── BCI IV-2a ─────────────────────────────────────────────────────────
    log.info("Checking BCI Competition IV-2a...")
    try:
    ...
    # ── PhysioNet MMIDB ───────────────────────────────────────────────────
    log.info("Checking PhysioNet MMIDB (2-subject probe)...")
    try:
        pdata = PhysionetMI()
        pparadigm = LeftRightImagery(fmin=4.0, fmax=40.0, resample=128.0)
        probe = pdata.subject_list[:2]
        for sid in probe:
            try:
                ...
                log.warning("  Subject %d: FAILED – %s", sid, e)
        log.info("PhysioNet MMIDB: probe OK (full dataset has %d subjects).",
                 len(pdata.subject_list))
    except Exception as e:
        log.error("PhysioNet download/load failed: %s", e)
        sys.exit(1)

    log.info("Stage 1 complete – all datasets verified.")
