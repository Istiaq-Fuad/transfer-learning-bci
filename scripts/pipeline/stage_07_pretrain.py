"""Stage 7 – Pretrain image backbone on PhysioNet MMIDB.
...
for use in Stages 8 and 9.

Output:
  <run-dir>/checkpoints/vit_pretrained_physionet_vit.pt
  <run-dir>/results/real_pretrain_physionet_vit.json
  <run-dir>/plots/stage_07_vit/  (pretraining loss/accuracy curves)

Usage::

    uv run python scripts/pipeline/stage_07_pretrain.py --run-dir runs/my_run
    uv run python scripts/pipeline/stage_07_pretrain.py --run-dir runs/my_run \\
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Stage 7: Pretrain image backbone on PhysioNet MMIDB.",
...
def main() -> None:
    ...
    log = setup_logging(run_dir, f"stage_07_pretrain_{bshort}.log")
    ...
    if checkpoint_path.exists() and out_path.exists():
        log.info("Checkpoint and results already exist – skipping Stage 7.")
        return
