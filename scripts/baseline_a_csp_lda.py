...
def make_predict_fn(n_components: int = 6):
    """Return a predict_fn for CSP + LDA.

    Each fold gets a fresh CSP + LDA to avoid data leakage.
    """
    def predict_fn(
        X_train: np.ndarray,
        y_train: np.ndarray,
...
def load_real_data(data_dir: str) -> dict[int, tuple[np.ndarray, np.ndarray]]:
    ...
    for subject_id in dataset.subject_list:
        try:
            X, y_labels, metadata = paradigm.get_data(
                dataset=dataset, subjects=[subject_id]
            )
            ...
            logger.info(
                "  Subject %d: X=%s, classes=%s", subject_id, X.shape, classes
            )
...
def main() -> None:
    ...
    parser.add_argument("--n-components", type=int, default=6, help="CSP components")
    ...
    predict_fn = make_predict_fn(n_components=args.n_components)
